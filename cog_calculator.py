...
def create_dropdown(frame, label_text, options, variable):
    label = ttk.Label(frame, text=label_text)
    label.grid(row=len(frame.winfo_children()), column=0, pady=5, sticky="w")
    dropdown = ttk.Combobox(frame, textvariable=variable, values=options, state="readonly", width=20)
    dropdown.grid(row=len(frame.winfo_children())-1, column=1, pady=5, sticky="w")
...
# Set the window size to be scalable
...
# Create dropdowns
create_dropdown(frame, "Select VC_MODEL:", vc_models, vc_model_var)
create_dropdown(frame, "Select VC_MOC:", vc_mocs, vc_moc_var)
create_dropdown(frame, "Select VC_FILL_TYPE:", vc_fill_types, vc_fill_type_var)
create_dropdown(frame, "Select VC_MOC_ELIMINATORS:", vc_eliminators, vc_eliminators_var)
create_dropdown(frame, "Select VC_SWEEPER_PIPING:", vc_sweeper_piping, vc_sweeper_piping_var)
create_dropdown(frame, "Select VC_INTAKE:", vc_intakes, vc_intake_var)
create_dropdown(frame, "Select VC_DISCHARGE:", vc_discharges, vc_discharge_var)

# Button to fetch and display combined COG details
fetch_button = ttk.Button(frame, text="Calculate Combined COG", command=display_combined_cog_details)
fetch_button.grid(row=len(frame.winfo_children()), column=0, columnspan=2, pady=20)

# Label to display the combined COG result
result_label = ttk.Label(frame, text="Combined COG Details: ")
result_label.grid(row=len(frame.winfo_children()), column=0, columnspan=2, pady=10)
